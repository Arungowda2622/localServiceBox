...
const AddConstruction = ({ navigation }) => {
  ...
  /* ---------------------------------- */
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "constructions"), (snap) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 🔥 Save in SAME collection (constructions)
      ...
      Alert.alert("Success", "Construction item added successfully!");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "constructions", editItem.id), {
        name: editItem.name,
  ...
  return (
    <View style={styles.container}>
      <Header title="Add Construction" navigation={navigation} />
      ...
      <Modal visible={addModal} animationType="slide">
        <View style={{ flex: 1, backgroundColor: "#F5F7FB" }}>
          ...
          {/* Header inside modal */}
          <Header title="Add Construction" navigation={{ goBack: () => setAddModal(false) }} />
